...
import { 
  Sparkles, Sliders, Check, Copy, RefreshCw, Star, Info, 
  ChevronRight, ArrowRight, CornerDownLeft, Volume2, Bookmark, Flame
} from 'lucide-react';
...
export default function ToneRewriter({ initialText, onSetInitialText, isDarkMode }: ToneRewriterProps) {
  ...
  const handleSmartRewrite = async () => {
    ...
    try {
      ...
      } else if (selectedTone === "academic" || selectedTone === "formal") {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 animate-fade-in transition-colors duration-300">
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-bold font-heading text-[#2D3330] dark:text-zinc-100 flex items-center gap-2">
            <Sliders className="w-6 h-6 text-[#4A6D5D] dark:text-emerald-400" />
      ...
      {/* Preset cards shortcuts to click and populate */}
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
          {samplePhrases.map((sp, idx) => (
            <button
              ...
              className="text-left p-3 rounded-xl border border-[#ECE7DC] dark:border-zinc-800 bg-white hover:bg-[#FAF7F2] dark:bg-zinc-900 dark:hover:bg-zinc-800/60 transition-colors cursor-pointer group"
            >
              <div className="text-xs font-bold text-[#4A6D5D] dark:text-emerald-400 group-hover:underline">{sp.label}</div>
              <p className="text-[11px] text-zinc-500 dark:text-zinc-400 mt-1 truncate">{sp.text}</p>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="lg:col-span-6 flex flex-col">
          <div className="bg-white dark:bg-zinc-900 border border-[#ECE7DC] dark:border-zinc-800/80 rounded-2xl p-5 flex-1 flex flex-col justify-between shadow-xs">
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-xs">
                  <div></div>
                  {rewriteInput && (
                    <button onClick={resetAll} className="text-red-500 dark:text-red-400 font-bold hover:underline">
                      សម្អាត (Clear)
                    </button>
                  )}
                </div>
                <textarea
                  ...
                  rows={4}
                  className="w-full p-4 rounded-xl border border-[#ECE7DC] dark:border-zinc-800 bg-[#FAF7F2]/40 dark:bg-zinc-950/20 text-sm focus:outline-hidden focus:ring-1 focus:ring-[#4A6D5D] text-[#2D3330] dark:text-zinc-200 placeholder-zinc-400 resize-none font-sans"
              ...
              {/* Tone/Voice Options */}
              <div className="space-y-2">
                <label className="text-xs font-bold text-[#587E6A] dark:text-zinc-400 uppercase tracking-wide block">
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  {[
                    ...
                    >
                      ...
                      <div className={`text-[9px] mt-1 ${selectedTone === t.key ? 'text-[#CEE2D7] dark:text-emerald-300' : 'text-zinc-400'}`}>
                        {t.desc}
                      </div>
                    </button>
                  ))}
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-[#587E6A] dark:text-zinc-400 uppercase block">
                  ទំហំប្រវែងឃ្លា (Target Length Sizing)
                </label>
                <div className="flex bg-[#FAF7F2] dark:bg-zinc-950 p-1 rounded-xl border border-[#ECE7DC]/60 dark:border-zinc-800/80">
                  {[
            ...
            <button
              ...
              className="mt-6 w-full flex items-center justify-center gap-2 bg-[#4A6D5D] hover:bg-[#3E5C4E] disabled:bg-[#4A6D5D]/50 text-white rounded-xl py-3.5 text-xs font-bold shadow-md shadow-[#4a6d5d]/10 cursor-pointer active:scale-[0.99] transition-all"
            >
              {isRewriting ? (
                <>
        ...
        <div className="lg:col-span-6 flex flex-col">
          <div className="bg-white dark:bg-zinc-900 border border-[#ECE7DC] dark:border-zinc-800/80 rounded-2xl p-5 flex-1 flex flex-col justify-between shadow-xs">
                  ...
                  {isRewriting ? (
                    ...
                    <>
                      ...
                      <p className="text-xs text-zinc-400 dark:text-zinc-600 mt-1 max-w-[280px]">
                        សូមបំពេញអត្ថបទខាងឆ្វេង រួចចុចប៊ូតុង "បំលែងទម្រង់ឃ្លាភាសាខ្មែរ" ដើម្បីមើលគំនិតផ្សាយពាណិជ្ជកម្មថ្មីៗ។
                      </p>
                    </>
                  )}
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between bg-emerald-50/50 dark:bg-emerald-950/10 px-4 py-3 rounded-xl border border-emerald-100/40 dark:border-emerald-800/30">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-semibold text-[#2D3330] dark:text-zinc-300">
                        កម្រិតទាក់ទាញទីផ្សារ (Marketing Appeal Index)៖
                      </span>
                    </div>
                    <span className="text-sm font-bold text-emerald-600 bg-white dark:bg-zinc-800 dark:text-emerald-300 px-3 py-1 rounded-lg border border-emerald-150/40 font-mono shadow-xs">
                      {rewriteResult.score}%
                    </span>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-zinc-50 to-[#FAF7F2] dark:from-zinc-950 dark:to-zinc-900 border border-[#ECE7DC]/60 dark:border-zinc-800 p-5 rounded-xl shadow-xs relative group">
                    <span className="absolute -top-2 left-3 px-2 py-0.5 bg-[#4A6D5D] text-white text-[9px] uppercase font-bold rounded-md">
                  ...
                  <div className="p-4 bg-orange-50/10 dark:bg-zinc-900 border border-amber-100/50 dark:border-zinc-800/80 rounded-xl space-y-2">
                    <span className="text-[10px] font-bold text-amber-800 dark:text-amber-400 uppercase tracking-wider flex items-center gap-1.5">
                  ...
                  {rewriteResult.benefits && rewriteResult.benefits.length > 0 && (
                    <div className="p-3.5 border border-zinc-100 dark:border-zinc-800 rounded-xl">
            ...
            {/* Hint alert element */}
            <div className="mt-5 p-3.5 bg-[#FAF7F2]/60 dark:bg-zinc-950/30 border border-[#ECE7DC]/40 dark:border-zinc-800/50 rounded-xl flex items-start gap-2 text-[10px] text-zinc-400 dark:text-zinc-500 leading-normal">
